...
import { deleteProduct, updateProduct, updateProductStatus } from "@/service/productService";
...
const ProductCard = ({ product, onUpdated }) => {
  ...
  async function handleDelete() {
    ...
    if (res?.status.code === "SUCCESS") {
      ...
      alert(res?.status?.message || "Failed to delete product.");
    }
  }
  ...
  async function handleToggleStatus() {
    ...
    const res = await updateProductStatus(idNumber, newStatus);
    ...
    if (res?.status.code === "SUCCESS") {
      ...
      alert(res?.status?.message || "Failed to update product status.");
    }
  }
  ...
  const handleEditSubmit = async () => {
    ...
    if (res?.status.code === "SUCCESS") {
      ...
      alert(res?.status?.message || "Failed to update product.");
    }
  };
  ...
  return (
    <>
      ...
      <Card className="relative overflow-hidden border border-zinc-200 bg-linear-to-br from-sky-50 via-white to-emerald-50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 rounded-2xl group">
        <div
          className={`absolute top-0 left-0 right-0 h-1 ${status === "ACTIVE" ? "bg-linear-to-r from-emerald-400 to-teal-500" : "bg-linear-to-r from-rose-400 to-red-500"}`}
        />

        <CardHeader className="flex flex-row justify-between items-start pt-6 pb-1">
        ...
        <CardContent className="space-y-4 pb-4">
          ...
          {description && (
            <p className="text-xs text-zinc-600 line-clamp-3">{description}</p>
          )}
          ...
          <div className="text-[11px] text-zinc-500 space-y-1.5 pt-3 border-t border-dashed border-zinc-200">
            ...
            <p className="flex justify-between">
              ...
              <span>{formattedDate(createAt)}</span>
            </p>
            <p className="flex justify-between">
              <span className="text-zinc-400">Updated</span>
              <span>{formattedDate(updateAt)}</span>
            </p>
          </div>
        </CardContent>
